...
const DeviceRedirectSettings = () => {
    ...
    // Separate the data loading effect to depend on a ref
    const isFirstLoad = useRef(true);

    useEffect(() => {
        // Skip the first automatic run
    ...
    // Effect to handle the initial load
    useEffect(() => {
        if (isFirstLoad.current) {
            loadRedirects();
        }
    ...
    const handleAddPageRedirect = async () => {
        ...
        try {
            ...
            // Adding all necessary parameters to ensure we get the correct result
    ...
    const handleSelectAll = (e) => {
        ...
        if (e.target.checked) {
            // Adding only current page items that aren't already selected
    ...
    // Function to handle resetting and reloading
